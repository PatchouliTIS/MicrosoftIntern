...
from ..ops.block_sparse_flash_attention import block_sparse_attention, triton_matmul
...
def search_pattern(q, k, head):
    ...
    def block_sparse(topK, block_size:int=64):
        ...
        topK = min(topK, block_num)
        block_topk = torch.topk(-qk, topK, -1).indices
    ...
    for ty, fc in [("stream_llm", stream_llm), ("vertical_and_slash", vertical_and_slash), ("block_sparse", block_sparse)]:
        if ty == "stream_llm":
            ...
            vs_list = [(10,100), (20,100), (20,500),(30, 800), (100, 750), (500, 700), (3500, 100)]
        else:
            vs_list = [(4, 32), (8, 64), (16, 64), (6, 64), (8, 128)]
...
def gather_last_q_vertical_slash_topk_vllm(self, q, k, v, head_id, block_index = None, seqlens = None):
    # kv_seq_len = k.size(2)
    head_dim = q.size(-1)
    ...
    def block_sparse_kernel(q, k, v, vertical_size=None, slash_size=None, block_index=None, seqlens=None):
        topk = 4
        return block_sparse_attention(q, k, v, topk, block_size_M=64, block_size_N=64,block_index=block_index, seqlens=seqlens)
    ...
    #     "block_sparse": block_sparse_kernel,
    # }[ty]
    # return vertical_and_slash_kernel(q, k, v, vertical_size=512, slash_size=1024)
    return block_sparse_kernel(q, k, v, vertical_size=10, slash_size=10, block_index=block_index, seqlens=seqlens)
    # return fc(q, k, v, vertical_size, slash_size, block_index, seqlens)


def gather_pyramidkv_topk_vllm(self, q, k, v, head_id, layer_idx):
    # TODO: Implement pyramidkv_topk_vllm
    kv_seq_len = k.size(2)
    head_dim = q.size(-1)
    q_len = q.shape[-2]
    bsz = q.shape[0]
    
    def vertical_and_slash_kernel(q, k, v, vertical_size, slash_size, layer_idx):
        # print("use pyramidkv")
        q_len = q.shape[-2]
        vertical_size, slash_size  = min(q_len, max(vertical_size, 30)), min(q_len, max(slash_size, 50))
        last_q = min(64, q_len)
        ...
        # TODO: Implement pyramidkv_attention
        ...
        if q_len < max_capacity_prompt:
            ...
            vertical_topk = torch.arange(q_len).reshape(1,1,-1).to(q.device)
        elif q_len < (max_capacity_prompt - window_size) * 2 :
            attn_weights = torch.matmul(q[..., -window_size:, :], k.transpose(2, 3)) / math.sqrt(head_dim)
            ...
            vertical_topk = attn_cache.topk(max_capacity_prompt - window_size, dim=-1).indices
            # add up tail
            tail_indices = torch.arange(q_len - window_size, q_len).reshape(vertical_topk.shape[0],vertical_topk.shape[1],-1).to(vertical_topk.device)
            vertical_topk = torch.cat((vertical_topk, tail_indices), dim=-1)
        else:
            attn_weights = torch.matmul(q[..., -window_size:, :], k.transpose(2, 3)) / math.sqrt(head_dim)
            mask = torch.full((window_size, window_size), torch.finfo(attn_weights.dtype).min, device=attn_weights.device)
            mask_cond = torch.arange(mask.size(-1), device=attn_weights.device)
            ...
            if pooling == 'avgpool':
                ...
                raise ValueError('Pooling method not supported')
            # attn_cache = attn_weights_sum
            vertical_topk = attn_cache.topk(max_capacity_prompt, dim=-1).indices
            # add up tail
            tail_indices = torch.arange(q_len - window_size, q_len).reshape(vertical_topk.shape[0],vertical_topk.shape[1],-1).to(vertical_topk.device)
            vertical_topk = torch.cat((vertical_topk, tail_indices), dim=-1)
    ...
    _, vertical_size, slash_size, _ = self.best_pattern[head_id]

    # use pyramidkv kernal
    return pyramidkv_kernel(q, k, v, layer_idx)
    # return vertical_and_slash_kernel(q, k, v, vertical_size, slash_size, layer_idx)
...
def minference_vllm_forward(
    ...
    def forward(
        ...
        def minference_prefill_func(
            ...
            for head in range(q.size(-2)):
                ...
                # out = self.gather_pyramidkv_topk_vllm(q_head, k_head, v_head, head, layer_idx)
    ...
    def forward_vllm_042(
        ...
        # def construct_tensor(block_size: int, top_k: int):
        #     if (block_size > top_k) :
        #         # 创建一个包含 0 到 7 的 tensor，形状为 (8, 1)
        #         row_indices = torch.arange(block_size - top_k).view(block_size - top_k, 1)
        #         # 创建一个包含 0 到 3 的 tensor，形状为 (1, 4)
        #         col_indices = torch.arange(1,top_k + 1).view(1, top_k)
        #         # 使用广播机制直接生成所需的矩阵
        #         base_matrix = row_indices + col_indices
        #         upper_matrix = torch.arange(top_k).repeat(top_k, 1)
        #         base_matrix = torch.cat((upper_matrix,base_matrix), 0)
        #         # 将 base_matrix 扩展为形状为 (1, 1, 8, 4)
        #         tensor = base_matrix.unsqueeze(0).unsqueeze(0).to('cuda')
        #     else :
        #         row_indices = torch.arange(block_size)
        #         # 创建一个包含 0 到 3 的 tensor，形状为 (1, 4)
        #         base_matrix = row_indices.repeat(block_size, 1)
        #         # 将 base_matrix 扩展为形状为 (1, 1, 8, 4)
        #         tensor = base_matrix.unsqueeze(0).unsqueeze(0).to('cuda')
        #     return tensor
        ...
        def minference_prefill_func(
            ...
            # translate tensor to (1, num_heads, seq_len, head_size)
            ...
            block_size_M = 64
            block_size_N = 64
            Mpad = block_size_M - (seq_len & (block_size_M - 1))
            Npad = block_size_N - (seq_len & (block_size_N - 1))
            # block_size = (seq_len + pad) // block_size_M
            # another_k = 1 if (int)(block_size * 0.1) == 0 else (int)(block_size * 0.1)
            # top_k = max(int(block_size * 0.4), 16)
            top_k = 8
            seqlens = torch.tensor([seq_len], dtype=torch.int32, device=q.device)
            
            # generate blocks
            # # top_k = 8
            # block_index = construct_tensor(block_size, top_k)
            
            
            
            # pad tensor
            q = torch.nn.functional.pad(q, [0, 0, 0, Mpad, 0, 0, 0, 0])
            k = torch.nn.functional.pad(k, [0, 0, 0, Npad, 0, 0, 0, 0])
            v = torch.nn.functional.pad(v, [0, 0, 0, Mpad, 0, 0, 0, 0])
            
            # calculate top_k blocks
            batch_size, num_heads, context_size, head_dim = q.shape
            num_kv_heads = k.shape[1]
            
            # p_pool = triton_matmul(q, k.transpose(-1, -2),block_size_M=64, block_size_N=64)
            # p_pool = torch.matmul(q, k.transpose(-1, -2))
            # p_pool = F.avg_pool2d(p_pool, kernel_size=64)
            # print(f"p_pool shape:{p_pool.shape}")
            block_count = 1
            # p_pool[..., :block_count] = torch.inf
            
            # top_least = context_size // block_size_M - windows_size if context_size // block_size_M - windows_size > 0 else 1
            query_pool = q.reshape((batch_size, num_heads, -1, block_size_M, head_dim)).mean(dim=-2)
            key_pool = k.reshape((batch_size, num_kv_heads, -1, block_size_N, head_dim)).mean(dim=-2)
            # query_pool = torch.max(q.reshape((batch_size, num_heads, -1, block_size_M, head_dim)), dim=-2).values
            # key_pool = torch.max(k.reshape((batch_size, num_heads, -1, block_size_M, head_dim)), dim=-2).values
            # prefix_blocks = query_pool.shape[-2] - windows_size if query_pool.shape[-2] - windows_size > 0 else 1
            # arange_M = torch.arange(p_pool.shape[-2], dtype=torch.int8, device=p_pool.device) * block_size_M
            # arange_N = torch.arange(p_pool.shape[-2], dtype=torch.int8, device=p_pool.device) * block_size_N
            
            # p_pool = torch.matmul(query_pool, key_pool.transpose(2,3))
            p_pool = triton_matmul(query_pool, key_pool.transpose(2,3), block_size_M=64, block_size_N=64)
            # p_pool = p_pool.where(arange_M[None, None, :, None] >= arange_N[None, None, None, :], -torch.inf)
            
            # mask = torch.ones_like(p_pool, dtype=torch.bool, device=p_pool.device)
            block_count = 1
            # mask[..., :block_count] = False
            # p_pool = p_pool.where(mask, torch.inf)
            p_pool[...,:block_count] = torch.inf
            # print(f"top_k: {top_k}\tactual size:{context_size // block_size_N}")
            # print(f"top_k: {top_k}\tactual size:{top_least}")
            top_k = min(top_k, context_size // block_size_M)
            # print(f"p_pool shape[-1]:{p_pool.shape[-1]}\ttop_k:{top_k}")
            block_index = torch.topk(p_pool, top_k, dim=-1).indices.to(torch.int32).sort(dim=-1).values
            # add_last = torch.full((batch_size, num_heads, block_index.shape[-2], 1), block_index.shape[-2] - 1, dtype=torch.int32, device=q.device)
            # block_index = torch.cat((block_index, add_last), dim=-1)
            
            # start_time = time.perf_counter()           
            
            out = self.gather_last_q_vertical_slash_topk_vllm(q, k, v, 32, block_index, seqlens)
            out = out.transpose(1, 2).squeeze(0)   # .contiguous()
            
            # elapsed_time = (time.perf_counter() - start_time) * 1000
            # print(f"Total Triton Elapsed time: {elapsed_time} ms")
            return out
            
            # for head in range(q.size(1)):   # q.size(-2)    
            #     q_head = q[:, head, :].unsqueeze(1)
            #     k_head = k[:, head, :].unsqueeze(1)
            #     v_head = v[:, head, :].unsqueeze(1)
            ...
            #     # k_head = k[:, head, :, :].unsqueeze(1)
            #     # v_head = v[:, head, :, :].unsqueeze(1)
            #     # block_index_head = block_index[:, head, :, :].unsqueeze(1)

            #     out = self.gather_last_q_vertical_slash_topk_vllm(q_head, k_head, v_head, head)
            #     # out = self.gather_last_q_vertical_slash_topk_vllm(q_head, k_head, v_head, head, block_index_head, seqlens)
            #     # out = self.gather_pyramidkv_topk_vllm(q_head, k_head, v_head, head, layer_idx)
            ...
            # # # elapsed_time = (time.perf_counter() - start_time) * 1000
            # # # print(f"OneByOne Triton Elapsed time: {elapsed_time} ms")
        ...
        #     k_org = key
        #     v_org = value
        #     k_org = k_org.transpose(0,1)
        #     v_org = v_org.transpose(0,1)
        #     if q.size(-2) != k.size(-2):
        #         k = repeat_kv(k, q.size(-2) // k.size(-2))
        #         v = repeat_kv(v, q.size(-2) // v.size(-2))
        #     q = q.transpose(0,1)
        ...
        #     max_capacity_prompt = (int)(q_len * 0.5)
        #     # window_size = triton.next_power_of_2((int)(max_capacity_prompt / 2))
        #     window_size = 32
        ...
        #         key_states = k
        #         value_states = v
        #         attn_weights = torch.matmul(q[..., -window_size:, :], key_states.transpose(-1, -2)) / math.sqrt(head_dim)
        #         # attn_weights = torch.einsum('...ij,...jk->...ik', query[..., -window_size:, :], key.transpose(2, 3)) / math.sqrt(head_dim)
        #         mask = torch.full((window_size, window_size), torch.finfo(attn_weights.dtype).min, device=attn_weights.device)
        #         mask_cond = torch.arange(mask.size(-1), device=attn_weights.device)
        ...
        #         # # pooling
        #         # 获取topk的索引
        #         topk_indices = attn_cache.topk(max_capacity_prompt - window_size, dim=-1).indices
        #         # # 转换形状为 [108, 128, 32] 以便于在第一个维度进行池化
        #         # indices = indices.permute(1, 2, 0)

        #         # # 使用 max_pool1d 进行池化操作
        #         # indices = F.max_pool1d(indices, kernel_size=4, stride=4)
        ...
        #         # 创建掩码
        #         mask = torch.zeros_like(key_states[:, :-window_size, :], dtype=torch.bool, device=key_states.device)
        #         mask.scatter_(-2, topk_indices.unsqueeze(-1).expand(-1, -1, head_dim), True)

        #         # 将不在topk索引中的元素设置为0
        #         key_states[:, :-window_size, :] *= mask
        #         value_states[:, :-window_size, :] *= mask
        ...
        # print(f">>>prefill_metadata:{attn_metadata.prefill_metadata}\n>>>decode_meta:{attn_metadata.decode_metadata}")
        if prefill_meta := attn_metadata.prefill_metadata:
            # Prompt run.
            if kv_cache is None or prefill_meta.block_tables.numel() == 0:
                # normal attention
                ...
                # out = minference_prefill_func(query, key_states, value_states)
                
                # start_time = time.perf_counter()
                out = minference_prefill_func(query, key, value)
                # end_time = time.perf_counter()
                # print(f"\nTime elapsed: {(end_time - start_time) * 1000}ms")
                # assert output[:num_prefill_tokens].shape == out.shape
                output[:num_prefill_tokens] = out
            else:
                # prefix-enabled attention
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            # end_time = time.perf_counter()
            # print(f"Decode Time elapsed: {(end_time - start_time) * 1000}ms")
