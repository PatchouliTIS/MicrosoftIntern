...
def llama_model_forward_vllm(
    ...
    count += (time.perf_counter() - start_time) * 1000        
    # print(f"minference time:{count}ms")
...
def llama_attn_forward_vllm(
    ...
    def llama_attn_forward_vllm(
        ...
        # attn_time = (time.perf_counter() - start_time) * 1000
        # print(f"minference attn time:{attn_time}ms")
...
def minference_patch_vllm(
    ...
    def update_module(m):
        if isinstance(m, Attention):
            ...
            # m.gather_pyramidkv_topk_vllm = gather_pyramidkv_topk_vllm.__get__(m, m_cls)
