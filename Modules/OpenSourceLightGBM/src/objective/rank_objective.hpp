...
#pragma omp parallel for schedule(static)
  ...
  inline void GetGradientsForOneQuery(data_size_t query_id, data_size_t cnt,
    ...
    //Log::Debug("query_id, inverse_max_dcg, worst_idx, best_score： %d, %f, %d, %f", query_id, inverse_max_dcg, worst_idx, best_score);
      ...
      for (data_size_t j = i + 1; j < cnt; ++j) {
        ...
        if (label[sorted_idx[i]] == label[sorted_idx[j]]) {
          if (secondary_label != nullptr)
          {
            ...
            //Log::Debug("Trigered secondary_labels: %f, %f", secondary_label[sorted_idx[i]], secondary_label[sorted_idx[j]]);
          }
          else
          {
        ...
        //Log::Debug("high_label, high_label_gain, low_label, low_label_gain, delta_pair_NDCG： %d, %f, %d, %f, %f", high_label, high_label_gain, low_label, low_label_gain, delta_pair_NDCG);
