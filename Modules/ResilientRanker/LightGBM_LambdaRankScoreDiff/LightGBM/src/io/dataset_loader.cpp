...
DatasetLoader::DatasetLoader(const Config& io_config, const PredictFunction& predict_fun, int num_class, const char* filename)
  ...
  Log::Info("dump_bin_boundaries_ = %d", dump_bin_boundaries_);
}
...
void DatasetLoader::SetHeader(const char* filename) {
  ...
  if (filename != nullptr) {
    ...
    // load doc idx
    if (config_.doc_id_column.size() > 0) {
      if (Common::StartsWith(config_.doc_id_column, name_prefix)) {
        std::string name = config_.doc_id_column.substr(name_prefix.size());
        if (name2idx.count(name) > 0) {
          doc_idx_ = name2idx[name];
          Log::Info("Using column %s as doc id", name.c_str());
        }
        else {
          Log::Fatal("Could not find doc column %s in data file", name.c_str());
        }
      }
      else {
        if (!Common::AtoiAndCheck(config_.doc_id_column.c_str(), &doc_idx_)) {
          Log::Fatal("doc_id_column is not a number,\n"
            "if you want to use a column name,\n"
            "please add the prefix \"name:\" to the column name");
        }
        Log::Info("Using column number %d as doc id", doc_idx_);
      }
      ignore_features_.emplace(doc_idx_);
    }
  }
  // load impression idx
  if (config_.impression_id_column.size() > 0) {
    if (Common::StartsWith(config_.impression_id_column, name_prefix)) {
      std::string name = config_.impression_id_column.substr(name_prefix.size());
      if (name2idx.count(name) > 0) {
        imp_idx_ = name2idx[name];
        Log::Info("Using column %s as impression id", name.c_str());
      }
      else {
        Log::Fatal("Could not find impression column %s in data file", name.c_str());
      }
    }
    else {
      if (!Common::AtoiAndCheck(config_.impression_id_column.c_str(), &imp_idx_)) {
        Log::Fatal("impression_id_column is not a number,\n"
          "if you want to use a column name,\n"
          "please add the prefix \"name:\" to the column name");
      }
      Log::Info("Using column number %d as impression id", imp_idx_);
    }
    ignore_features_.emplace(imp_idx_);
  }
...
// dump bin boundaries for each feature
void DumpBinBoundaries(int num_features, const std::vector<std::unique_ptr<BinMapper>>& bin_mappers, const std::string& mode, int offset = 0) {
  std::cout << "=================================================" << std::endl;
  std::cout << "bin boundaries for " << mode << " mode, number of features: " << num_features << std::endl;
  for (int i = 0; i < num_features; ++i) {
    std::cout << "----- feature " << i + offset << std::endl;
    if (bin_mappers[i] == nullptr) {
      continue;
    }
    const auto& binmappper = bin_mappers[i];
    std::cout << "bin type: " << binmappper->bin_type() 
              << ", num bin: " << binmappper->num_bin() 
              << ", missing_type: " << binmappper->missing_type() 
              << ", is_trivial: " << binmappper->is_trivial() 
              << ", sparse_rate: " << binmappper->sparse_rate()
              << ", bin info: " << binmappper->bin_info_string()
              << std::endl;
    const auto& boundaries = binmappper->bin_boundaries();
    int print_blks = boundaries.size() / 10;
    for (int j = 0; j < print_blks; j++) {
      for (int k = 0; k < 10; k++) {
        std::cout << std::setprecision(9) << boundaries[j * 10 + k] << ", ";
      }
      std::cout << std::endl;
    }
    for (int j = print_blks * 10; j < boundaries.size(); j++) {
      std::cout << std::setprecision(9) << boundaries[j] << ", ";
    }
    std::cout << std::endl;
  }
  std::cout << "=================================================" << std::endl;
...
void DatasetLoader::ConstructBinMappersFromTextData(int rank, int num_machines,
  ...
  if (num_machines == 1) {
    ...
    // dump bin boundaries for each feature
    if (dump_bin_boundaries_) {
      DumpBinBoundaries(static_cast<int>(sample_values.size()), bin_mappers, "single machine");
    }
  } else {
    // start and len will store the process feature indices for different machines
    // machine i will find bins for features in [ start[i], start[i] + len[i] )
    ...
    // dump bin boundaries for each feature
    //DumpBinBoundaries(len[rank], bin_mappers, "multiple machine - local", start[rank]);
    ...
    // dump bin boundaries for each feature
    if (dump_bin_boundaries_) {
      DumpBinBoundaries(dataset->num_total_features_, bin_mappers, "multiple machine");
    }
  }
    ...
    for (data_size_t i = 0; i < dataset->num_data_; ++i) {
      ...
      for (auto& inner_data : oneline_features) {
        ...
        if (feature_idx >= 0) {
          ...
          } else if (inner_data.first == doc_idx_) {
