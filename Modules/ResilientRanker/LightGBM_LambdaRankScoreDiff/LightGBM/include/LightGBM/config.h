...
struct Config {
  ...
  */
  ...
  // desc = if dump bin boundaries in log
  bool dump_bin_boundary = false;
  ...
  // desc = used to control the weight of scorediff in loss function
  double scorediff_alpha = 0.0;

  // check = >=0.0
  // desc = used only in ``lambdarank_scorediff`` application
  // desc = used to control the work point of scorediff in loss function
  double scorediff_beta = 0.0;

  // check = >=0.0
  // desc = used only in ``lambdarank_rankdiff`` application
  // desc = used to control the weight of rankdiff in loss function
  double rankdiff_alpha = 0.0;

  // type = int or string
  // alias = doc_id
  // desc = used to specify the doc id column
  // desc = use number for index, e.g. ``doc_id=0`` means column\_0 is the doc id
  // desc = add a prefix ``name:`` for column name, e.g. ``doc_id=name:doc``
  // desc = **Note**: works only in case of loading data directly from file
  // desc = **Note**: index starts from ``0`` and it doesn't count the label column when passing type is ``int``, e.g. when label is column\_0, and doc_id is column\_1, the correct parameter is ``doc_id=0``
  std::string doc_id_column = "";

  // type = int or string
  // alias = impression_id, imp_id
  // desc = used to specify the impression id column
  // desc = use number for index, e.g. ``imp_id=0`` means column\_0 is the impression id
  // desc = add a prefix ``name:`` for column name, e.g. ``imp_id=name:impression``
  // desc = **Note**: works only in case of loading data directly from file
  // desc = **Note**: index starts from ``0`` and it doesn't count the label column when passing type is ``int``, e.g. when label is column\_0, and imp_id is column\_1, the correct parameter is ``imp_id=0``
  std::string impression_id_column = "";

  // desc = used only in ``lambdarank_scorediff2`` application
  // desc = set this to ``true`` to normalize the lambdas for score diff part
  // desc = set this to ``false`` to use raw score diff gradient
  bool normalize_gradient_scorediff = true;
