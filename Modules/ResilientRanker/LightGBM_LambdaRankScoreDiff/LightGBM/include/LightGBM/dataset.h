...
class Metadata {
  ...
  */
  ...
  void SetDoc(const data_size_t* doc, data_size_t len);

  void SetImpression(const data_size_t* impression, data_size_t len);
  ...
  */
  ...
  /*!
  * \brief Set Doc Id for one record
  * \param idx Index of this record
  * \param value Doc Id value of this record
  */
  inline void SetDocAt(data_size_t idx, doc_id_t value) {
    docs_[idx] = value;
  }

  /*!
  * \brief Set Impression Id for one record
  * \param idx Index of this record
  * \param value Impression Id value of this record
  */
  inline void SetImpressionAt(data_size_t idx, data_size_t value) {
    impressions_[idx] = value;
    last_impression_id_ = idx; //?
    total_set_impression_++;
  }

  /*!
  * \brief Get pointer of doc pair containers
  * \return Pointer of doc pair containers
  */
  inline const DocPairContainer* doc_pairs() const {
    if (!doc_pairs_.empty()) {
      return doc_pairs_.data();
    }
    else {
      return nullptr;
    }
  ...
  */
  ...
  /*!
  * \brief Get number of impressions for each query, if not exists, will return nullptr
  *        we assume data will order first by query, second by impression
  *        num_impressions[i] is the data indices for query i.
  * \return Pointer of data number of impressions for each query
  */
  inline const data_size_t* impression_cnts() const {
    if (!num_impressions_.empty()) {
      return num_impressions_.data();
    }
    else {
      return nullptr;
    }
 ...
 private:
  ...
  void LoadDocIDs();
  /*! \brief Load impression id from file */
  void LoadImpressionIDs();
  /*! \brief Load query boundaries from file */
  void LoadQueryBoundaries();
  /*! \brief Load query wights */
  void LoadQueryWeights();
  /*! \brief Load identical doc pairs */
  void LoadDocPairs();
  /*! \brief Load number of impressions */
  void LoadNumImpressions();
  ...
  /*! \brief Number of doc id */
  data_size_t num_docs_;
  /*! \brief Doc data */
  std::vector<doc_id_t> docs_;
  /*! \brief Doc pairs for each query. One doc pair has the same doc id. */
  std::vector<DocPairContainer> doc_pairs_;
  /*! \brief Number of impressions */
  std::vector<data_size_t> impressions_;
