...
-  ``dump_bin_boundary`` :raw-html:`<a id="dump_bin_boundary" title="Permalink to this parameter" href="#dump_bin_boundary">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool

   -  if dump bin boundaries in log
...
-  ``rankdiff_alpha`` :raw-html:`<a id="rankdiff_alpha" title="Permalink to this parameter" href="#rankdiff_alpha">&#x1F517;&#xFE0E;</a>`, default = ``0.0``, type = double, constraints: ``rankdiff_alpha >= 0.0``
   ...
   -  used only in ``lambdarank_rankdiff`` application

   -  used to control the weight of rankdiff in loss function

-  ``doc_id_column`` :raw-html:`<a id="doc_id_column" title="Permalink to this parameter" href="#doc_id_column">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = int or string, aliases: ``doc_id``

   -  used to specify the doc id column

   -  use number for index, e.g. ``doc_id=0`` means column\_0 is the doc id

   -  add a prefix ``name:`` for column name, e.g. ``doc_id=name:doc``

   -  **Note**: works only in case of loading data directly from file

   -  **Note**: index starts from ``0`` and it doesn't count the label column when passing type is ``int``, e.g. when label is column\_0, and doc_id is column\_1, the correct parameter is ``doc_id=0``

-  ``impression_id_column`` :raw-html:`<a id="impression_id_column" title="Permalink to this parameter" href="#impression_id_column">&#x1F517;&#xFE0E;</a>`, default = ``""``, type = int or string, aliases: ``impression_id``, ``imp_id``

   -  used to specify the impression id column

   -  use number for index, e.g. ``imp_id=0`` means column\_0 is the impression id

   -  add a prefix ``name:`` for column name, e.g. ``imp_id=name:impression``
...
-  ``normalize_gradient_scorediff`` :raw-html:`<a id="normalize_gradient_scorediff" title="Permalink to this parameter" href="#normalize_gradient_scorediff">&#x1F517;&#xFE0E;</a>`, default = ``false``, type = bool
   ...
   -  set this to ``true`` to normalize the lambdas for score diff part

   -  set this to ``false`` to use raw score diff gradient
